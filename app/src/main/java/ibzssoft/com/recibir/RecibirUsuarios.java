...
public class RecibirUsuarios {
    ...
    public RecibirUsuarios(Context context) {
    ...
    public void cargarPreferenciasConexion(){
        ...
        ws=extraerConfiguraciones.get(context.getString(R.string.key_ws_usuarios),context.getString(R.string.pref_ws_usuarios));
    ...
    public void ejecutartarea(){
        RecibirUsuariosTask taskRecibirUsuarios= new RecibirUsuariosTask();
        taskRecibirUsuarios.execute();
    }

    private class RecibirUsuariosTask extends AsyncTask<String,Integer,Boolean> {
        ...
        protected void onPreExecute() {
            ...
            progress.setTitle("Descargando Usuarios");
        ...
        protected Boolean doInBackground(String... params) {
            ...
            try {
                ...
                for (int i = 0; i < respJSON.length(); i++) {
                    ...
                    Usuario usr = gson.fromJson(obj.toString(), Usuario.class);
                    if (helper.existeUsuario(usr.getCodusuario())) {
                        helper.modificarUsuario(usr);
                    } else {
                        helper.crearUsuario(usr);
                    }
                    count++;
                    publishProgress(count);
                    System.out.println("Usuario recibido: "+usr.toString());
                }
                helper.close();
                Thread.sleep(100);
                result = true;
        ...
        protected void onPostExecute(Boolean s) {
            if (progress.isShowing()) {
                progress.dismiss();
                if(s){
                    RecibirVendedores recibirVendedores= new RecibirVendedores(context);
                    recibirVendedores.ejecutartarea();
                }
            }
        }
    }
