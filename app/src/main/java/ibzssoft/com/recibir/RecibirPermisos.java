...
public class RecibirPermisos {
    ...
    public RecibirPermisos(Context context) {
    ...
    public void cargarPreferenciasConexion(){
        ...
        ws=extraerConfiguraciones.get(context.getString(R.string.key_ws_permisos),context.getString(R.string.pref_ws_permisos));
    ...
    private class RecibirPermisosTask extends AsyncTask<String,Integer,Boolean> {
        ...
        protected void onPreExecute() {
            ...
            progress.setTitle("Descargando Permisos");
        ...
        protected Boolean doInBackground(String... params) {
            ...
            try
            {
                ...
                if(respJSON.length()>0){
                    helper.vaciarPermisos();
                    for(int i=0; i<respJSON.length(); i++)
                    {
                        JSONObject obj = respJSON.getJSONObject(i);
                        Permiso per=gson.fromJson(obj.toString(), Permiso.class);
                        helper.crearPermiso(per);
                        count++;
                        publishProgress(count);
                        System.out.println("Permiso recibido: "+per.toString());
                    }
                }
                helper.close();
                Thread.sleep(100);
        ...
        protected void onPostExecute(Boolean s) {
            if(progress.isShowing()){
                progress.dismiss();
                if(s){
                    RecibirPermisosTrans recibirPermisosTrans= new RecibirPermisosTrans(context);
                    recibirPermisosTrans.ejecutartarea();
                }
            }
        }
    }
