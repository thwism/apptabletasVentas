...
public class RecibirBanco {
    ...
    public RecibirBanco(Context context) {
    ...
    public void cargarPreferenciasConexion(){
        ...
        ws=extraerConfiguraciones.get(context.getString(R.string.key_ws_bancos),context.getString(R.string.pref_ws_bancos));
    ...
    private class RecibirBodegasTask extends AsyncTask<String,Integer,Boolean> {
        ...
        protected void onPreExecute() {
            ...
            progress.setTitle("Descargando Bancos");
        ...
        protected Boolean doInBackground(String... params) {
            ...
            try
            {
                ...
                for(int i=0; i<respJSON.length(); i++)
                {
                    ...
                    Banco per=gson.fromJson(obj.toString(), Banco.class);
                    count++;
                    if(helper.existeBanco(per.getIdbanco())){
                        helper.modificarBanco(per);
                    }else helper.crearBanco(per);
                    //System.out.println("Banco recibido: "+per.toString());
        ...
        protected void onPostExecute(Boolean s) {
            if(progress.isShowing()){
                progress.dismiss();
                if(s){
                    RecibirFormasCobro recibirFormasCobro= new RecibirFormasCobro(context);
                    recibirFormasCobro.ejecutartarea();
                }
            }
        }
    }
