...
public class RecibirGNTrans {
    ...
    public RecibirGNTrans(Context context) {
    ...
    public void cargarPreferenciasConexion(){
        ...
        ws=extraerConfiguraciones.get(context.getString(R.string.key_ws_gntrans),context.getString(R.string.pref_ws_gntrans));
    ...
    public void ejecutartarea(){
        RecibirGNTranssTask taskRecibirGNTranss= new RecibirGNTranssTask();
        taskRecibirGNTranss.execute();
    }

    private class RecibirGNTranssTask extends AsyncTask<String,Integer,Boolean> {
        ...
        protected void onPreExecute() {
            ...
            progress.setTitle("Descargando GNTrans");
            ...
            try
            {
                ...
                if(respJSON.length()>0){
                    for(int i=0; i<respJSON.length(); i++)
                    {
                        ...
                        GNTrans gnt=gson.fromJson(obj.toString(), GNTrans.class);
                        if(helper.existeGNTrans(gnt.getCodtrans())){
                            helper.modificarGNTrans(gnt);
                        }else helper.crearGNTrans(gnt);
                        count++;
                        System.out.println("GNTrans recibido: "+gnt.toString());
        ...
        protected void onPostExecute(Boolean s) {
            if(progress.isShowing()){
                progress.dismiss();
                if(s){
                    RecibirGrupos recibirGrupos= new RecibirGrupos(context);
                    recibirGrupos.ejecutartarea();
                }
            }
        }
    }
