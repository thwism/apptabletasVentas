...
import ibzssoft.com.modelo.DetalleBalance;

public class Balance extends AppCompatActivity implements
        ...
        DatePickerDialog.OnDateSetListener
{
    ...
    private String url;
    private String ws;
    private ArrayList<DetalleBalance> items = new ArrayList<DetalleBalance>();
        ...
        buscar.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(validarFechaMenor(date1.getText().toString(),date2.getText().toString())){
                    ...
                    RecibirInfoCierreTask recibirHistorialTask =new RecibirInfoCierreTask();
                    recibirHistorialTask.execute(ip,port,url,ws,date1.getText().toString(),date2.getText().toString());
                }
            }
        });
    }
    public void cargarPreferenciasConexion(){
        ExtraerConfiguraciones extraerConfiguraciones= new ExtraerConfiguraciones(Balance.this);
        ...
        ws=extraerConfiguraciones.get(getString(R.string.key_ws_balance),getString(R.string.pref_ws_balance));
    }
    ...
    public void agregarFilasTabla(ArrayList<DetalleBalance> detalles){
        encerarArreglos();
        int pst=0;
        TableRow fila;
        if(!detalles.isEmpty()){
            do{
                if (detalles.get(pst).getTipoCuenta().equals("1") || detalles.get(pst).getTipoCuenta().equals("2") || detalles.get(pst).getTipoCuenta().equals("3")) {
                    ...
                    codigo[pst].setText(detalles.get(pst).getCodCuenta());
                    //codigo[pst].setText(new ParseDates().changeDateToStringSimple1(new ParseDates().changeStringToDateSimpleFormat(detalles.get(pst).getFechaTrans())));
                    ...
                    descripciones[pst].setText(detalles.get(pst).getNombreCuenta());
                    ...
                    transacciones[pst].setText(detalles.get(pst).getValor());
                    ...
                    fechas[pst].setText(detalles.get(pst).getNivel());
                    ...
                    can_total[pst].setText(String.valueOf(detalles.get(pst).getTipoCuenta()));
                    ...
                    precios[pst].setText(String.valueOf(detalles.get(pst).getIdCuentaSuma()));
    ...
    private class RecibirInfoCierreTask extends AsyncTask<String,Integer,Boolean> {
        private ProgressDialog progress;

        @Override
        protected void onPreExecute() {
            super.onPreExecute();
            progress = new ProgressDialog(Balance.this);
            progress.setTitle("Descargando Balance");
        ...
        protected Boolean doInBackground(String... params) {
            ...
            System.out.println("Consulta historial: "+"http://" +ip_param+ ":" + port_param+ url_param+ ws_param +"/"+ fecha1 + "/" + fecha2);
            HttpGet del = new HttpGet("http://" +ip_param+ ":" + port_param+ url_param+ ws_param +"/"+ fecha1 + "/" + fecha2);
            ...
            try {
                ...
                for (int i = 0; i < respJSON.length(); i++) {
                    ...
                    DetalleBalance per = gson.fromJson(obj.toString(), DetalleBalance.class);
                    items.add(per);
                    System.out.println("Info recibida: "+per.toString());
                }
                Thread.sleep(100);
                result = true;
            } catch (Exception ex) {
        ...
        protected void onPostExecute(Boolean s) {
            if (progress.isShowing()) {
                ...
                if(s){
                    ...
                    Toast ts = Toast.makeText(Balance.this, "No se puede descargar el historial", Toast.LENGTH_LONG);
                    ts.show();
                }
            }
        }
