...
import ibzssoft.com.adaptadores.TransCobroAdapter;
...
import ibzssoft.com.ishidamovile.cobro.crear.Cobro;
import ibzssoft.com.ishidamovile.cobro.crear.CobroY;
...
public class TabCobroAll extends Fragment implements SearchView.OnQueryTextListener  {
    private RecyclerView mRecyclerView;
    private TransCobroAdapter mAdapter;
    private ArrayList<Transaccion> transacciones;
    private TextView countTrans;
    private String [] accesos;
    private int opcion,orden,comodin;
    private String empresa,grupo;
    private boolean deleteTrans;
    private static final String catalogo="COBRO";
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.content_cobro_all, container, false);
        mRecyclerView = (RecyclerView) view.findViewById(R.id.my_recycler_view);
        countTrans= (TextView) view.findViewById(R.id.textCountCobros);
    ...
    public void extraerParametros(){
        ...
        this.deleteTrans=ext.getBoolean(getString(R.string.key_conf_supr_trans),false);
    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        mAdapter = new TransCobroAdapter(getActivity(), this.transacciones,this.empresa,this.opcion);
    ...
    private void prepareSortSpinner() {
        ...
        sortSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                mAdapter.animateTo(sortList(transacciones, position));
            }
    ...
    private List<Transaccion> sortList(List<Transaccion> models,int sort) {
        switch (sort){
            case 0: //ordenar por nro de transaccion
                Collections.sort(models, new Comparator<Transaccion>() {
                    ...
                    public int compare(Transaccion transaccion, Transaccion t1) {
                        return new Integer(t1.getNumTransaccion()).compareTo(new Integer(transaccion.getNumTransaccion()));
                    }
                });
                break;
            case 1: //ordenar por nombre de cliente
                Collections.sort(models, new Comparator<Transaccion>() {
                    @Override
                    public int compare(Transaccion transaccion, Transaccion t1) {
                        return transaccion.getCliente_id().compareTo(t1.getCliente_id());
                    }
                });
                break;
            case 2: //ordenar por fecha
                Collections.sort(models, new Comparator<Transaccion>() {
                    @Override
                    public int compare(Transaccion transaccion, Transaccion t1) {
                        return t1.getFecha_grabado().compareTo(transaccion.getFecha_grabado());
                    }
                });
                break;
            case 3: //ordenar por transaccion
                Collections.sort(models, new Comparator<Transaccion>() {
                    @Override
                    public int compare(Transaccion transaccion, Transaccion t1) {
                        return t1.getReferencia().compareTo(transaccion.getReferencia());
                    }
                });
                break;
        }
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        MenuItem register = menu.findItem(R.id.action_eliminar_no_send);
        if(deleteTrans)register.setVisible(true);
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        inflater.inflate(R.menu.menu_tab_cobro, menu);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.action_enviar:
                ...
                quitDialog.setTitle("Seguro deseas enviar todos los cobros no enviados?");
            ...
            case R.id.action_eliminar_send:
                ...
                quitDialog2.show();
                break;
            case R.id.action_eliminar_no_send:
                android.support.v7.app.AlertDialog.Builder quitDialog3 = new android.support.v7.app.AlertDialog.Builder(getActivity());
                quitDialog3.setTitle("Seguro deseas eliminar todos los cobros no enviados?");
                quitDialog3.setPositiveButton("Eliminar", new DialogInterface.OnClickListener() {
                ...
                quitDialog3.setNegativeButton("Cancelar", new DialogInterface.OnClickListener() {
    ...
    private List<Transaccion> filter(List<Transaccion> models, String query) {
        ...
        for (Transaccion model : models) {
            ...
            final String codtrans = model.getReferencia().toLowerCase();
            if (cliente.contains(query)||codtrans.contains(query)) {
        ...
        alertDialogBuilder.setPositiveButton("Continuar", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                if(gntr.length>0){
                    ...
                    Intent intent;
                    switch (gno){
                        case YOLITA2014: intent = new Intent(getContext(),CobroY.class);break;
                        case YOLITA2015: intent = new Intent(getContext(),CobroY.class);break;
                        case AUTO: intent = new Intent(getContext(),CobroY.class);break;
                        case CODEL: intent = new Intent(getContext(),CobroY.class);break;
                        default:intent = new Intent(getContext(),Cobro.class);break;
                    }
            ...
            protected String doInBackground(String... params) {
                ...
                for(Transaccion trans: transacciones){
                    if(trans.getBand_enviado()==0){
                        ...
                        System.out.println("http://"+ip+":"+port+url+ws+"/"+ generarTramaEnvio(trans.getId_trans()));
    ...
    class EliminarTransaccionesTask extends AsyncTask<String,Integer,Boolean> {
        ...
        protected Boolean doInBackground(String... params) {
            boolean result;
            int filter =Integer.parseInt(params[0]);
            try{
                DBSistemaGestion helper = new DBSistemaGestion(getActivity());
                Cursor cursor = helper.consultarTransacciones(accesos, filter, catalogo,orden,comodin);
                if (cursor.moveToFirst()){
                    do{
